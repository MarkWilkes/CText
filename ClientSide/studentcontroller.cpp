...
void StudentController::viewCourseButtonClicked()
{
    ...
    QString termsNCourses = c->sendRequest("getRegistedCourseRequest|", studentID);
    if(termsNCourses == "empty")
    {
        ...
        QStringList terms = termsNCourses.split(":", QString::SkipEmptyParts);    //2014FALL,COMP3004,COMP4000 for one line
        ...
        for(int i = 0; i < terms.size(); i++)
        {
            ...
            for(int courseNum = 1; courseNum < term.size(); courseNum++)
            {
                ...
                QStringList courseInfos = courseInfo.split("|");

                QString CourseID,CourseName,BookList;
                CourseID = courseInfos[0];
                CourseName = courseInfos[1];
                BookList = courseInfos[2];
                Course *course = new Course(CourseID, CourseName);
                courses.push_back(course);
                //set the value to the gui
                QTreeWidgetItem *courseWidget = addChild(termWidget, CourseName, "");
...
void StudentController::addCartButtonClicked(){
    ...
    for(int i = 0; i < items.size(); i++){
        ...
        if(items[i]->parent()->parent()->parent()->parent()->parent() == 0){
            //qDebug() << "section" << endl;
            QString itemName = items[i]->text(0);
