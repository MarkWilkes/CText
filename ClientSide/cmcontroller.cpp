...
void CmController::viewContentButtonClicked() {
    ...
    for(int j = 0; j < BookIDs.size(); j++)
    {
        ...
        bookInfos = c->sendRequest("getBookInfoRequest|",
                                   temp.append("|").append(BookIDs[j]));
...
void CmController::submitButtonClicked() {
    ...
    //check for blanks
    if(ui->title_textbox->text().isEmpty() || ui->price_textbox->text().isEmpty()){
    ...
    //---ADD BOOK---
        ...
        bookPrice = ui->price_textbox->text();
    ...
    //---ADD CHAPTER---
        ...
        chapterPrice = ui->price_textbox->text();
    ...
    //---ADD SECTION---
        ...
        sectionPrice = ui->price_textbox->text();
